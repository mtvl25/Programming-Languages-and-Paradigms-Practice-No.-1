...
-----------------------------------------------------------------------
-- | ln1pSeries
-- ¿Qué hace? Aproxima ln(1+x) con N términos:
--            sum_{n=1}^{N} (-1)^{n+1} * x^n / n    con |x| < 1
-- Entradas: x :: Double (debe cumplir |x| < 1), nTerms :: Int
-- Salidas:  Double
-----------------------------------------------------------------------
ln1pSeries :: Double -> Int -> Double
ln1pSeries x nTerms =
  if nTerms <= 0 then 0.0 else go 1 0.0
  where
    go n acc =
      if n > nTerms
        then acc
        else
          let term = (altSign (n + 1)) * (pow x n) / fromIntegral n
          in go (n + 1) (acc + term)
